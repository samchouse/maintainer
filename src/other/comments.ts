...
export const YSYL = () => ({
    tag: 'merge',
    status:
        'After a week, no one has reviewed the PR 😞. A maintainer will be reviewing the PR in the next few days and will either merge it or request revisions. Thank you for your patience!'
});
...
export const PingReviewers = (
    ...
    status: `🔔 ${names
        ...
        .join(
            ' '
        )} — please [review this PR](${reviewLink}) in the next few days. Be sure to explicitly select **\`Approve\`** or **\`Request Changes\`** in the GitHub UI so I know what's going on.`
});
...
export const PingReviewersTooMany = (names: readonly string[]) => ({
    ...
    status: `⚠️ There are too many reviewers for this PR change (${
        names.length
...
export const PingStaleReviewer = (
...
export const AskForAutoMergePermission = (
    user: string,
    otherOwners: string[]
) => ({
    tag: `merge-offer`,
