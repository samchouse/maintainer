import { GetPRInfo } from '../graphql/queries/pr-query';
import {
    PR as PRQueryResult,
    PR_repository_pullRequest as GraphqlPullRequest,
...
import { PullRequestState } from '../graphql/graphql-global-types';
...
export interface PrInfo {
     ...
     * a prior commit.
     */
    readonly reviewersWithStaleReviews: ReadonlyArray<{
        reviewer: string;
        reviewedAbbrOid: string;
        date: string;
    }>;
...
function getHeadCommit(pr: GraphqlPullRequest) {
    return pr.commits.nodes?.filter((c) => c?.commit.oid === pr.headRefOid)?.[0]
        ?.commit;
}
...
export async function deriveStateForPR(
    ...
    PrInfo | BotFail | BotError | BotEnsureRemovedFromProject | BotNoPackages
> {
    ...
    if (prInfo.author == null)
        return botError(prInfo.number, 'PR author does not exist');
    ...
    if (headCommit == null)
        return botError(prInfo.number, 'No head commit found');
    ...
    if (pkgInfoEtc instanceof Error)
        return botError(prInfo.number, pkgInfoEtc.message);
    ...
    const freshReviewsByState = partition(
        noNulls(prInfo.reviews?.nodes),
        (r) => r.state
    );
    ...
    const lastCommentDate =
        getLastCommentishActivityDate(prInfo.timelineItems, prInfo.reviews) ||
        lastPushDate;
    ...
    return {
        ...
        mergeIsRequested:
            !!prInfo.comments.nodes &&
            usersSayReadyToMerge(
                noNulls(prInfo.comments.nodes),
                dangerLevel.startsWith('Scoped')
                    ? [author, ...allOwners]
                    : [author],
                [
                    createdDate,
                    lastPushDate,
                    reopenedDate,
                    reviewAnalysis.firstApprovalDate
                ]
            ),
        stalenessInDays: Math.min(
            ...activityDates.map((date) => daysSince(date || lastPushDate, now))
        ),
        lastPushDate,
        reopenedDate,
        lastCommentDate,
        maintainerBlessed: lastBlessing
            ? lastBlessing.getTime() > lastPushDate.getTime()
            : false,
...
function getReopenedDate(
    timelineItems: PR_repository_pullRequest_timelineItems
) {
    const lastItem = findLast(
        timelineItems.nodes,
        (item): item is ReopenedEvent | ReadyForReviewEvent =>
            item?.__typename === 'ReopenedEvent' ||
            item?.__typename === 'ReadyForReviewEvent'
    );
...
function getLastCommentishActivityDate(
    timelineItems: PR_repository_pullRequest_timelineItems,
...
function getLastMaintainerBlessingDate(
    timelineItems: PR_repository_pullRequest_timelineItems
) {
    const lastColumnChange = findLast(
        timelineItems.nodes,
        (item): item is MovedColumnsInProjectEvent => {
            return (
                item?.__typename === 'MovedColumnsInProjectEvent' &&
                authorNotBot(item!)
            );
        }
    );
...
async function getPackageInfosEtc(
    ...
    { pkgInfo: PackageInfo[]; popularityLevel: PopularityLevel } | Error
> {
    const infos = new Map<string | null, FileInfo[]>();
    for (const path of paths) {
        const [pkg, fileInfo] = await categorizeFile(
            path,
            async (oid: string = headId) => fetchFile(`${oid}:${path}`)
        );
    ...
    for (const [name, files] of infos) {
        const owners = !name
            ? null
            : await getOwnersOfPackage(name, 'master', fetchFile);
        const newOwners = !name
            ? null
            : !paths.includes(`types/${name}/index.d.ts`)
            ? owners
    ...
    return {
        pkgInfo: result,
        popularityLevel: downloadsToPopularityLevel(maxDownloads)
    };
}
...
async function categorizeFile(
    path: string,
    contents: (oid?: string) => Promise<string | undefined>
    ...
    switch (suffix || '') {
        case 'd.ts':
            return [pkg, { path, kind: 'definition' }];
        case 'ts':
        case 'tsx':
            return [pkg, { path, kind: 'test' }];
        case 'md':
            return [pkg, { path, kind: 'markdown' }];
        default:
            ...
            return [
                ...
                {
                    path,
                    kind: suspect ? 'package-meta' : 'package-meta-ok',
                    suspect
                }
            ];
    }
...
interface ConfigSuspicious {
    (
        path: string,
        getContents: (oid?: string) => Promise<string | undefined>
...
configSuspicious['OTHER_FILES.txt'] = (contents) =>
    // not empty
    contents.length === 0 ? 'empty' : undefined;
configSuspicious['package.json'] = makeJsonCheckerFromCore({ private: true }, [
    'dependencies',
    'types',
    'typesVersions'
]);
...
function usersSayReadyToMerge(
    comments: PR_repository_pullRequest_comments_nodes[],
    users: string[],
    sinceDates: (Date | undefined)[]
) {
    const sinceDate = Math.max(
        ...sinceDates.map((date) => date?.getTime() || 0)
    );
...
function analyzeReviews(
    ...
    const staleReviewAuthorsByCommit = new Map<
        string,
    ...
    for (const r of reviews) {
        ...
        if (r.commit.oid !== headCommitOid) {
            ...
            if (!hasUpToDateReview.has(r.author.login)) {
                staleReviewAuthorsByCommit.set(r.commit.abbreviatedOid, {
            ...
            lastReviewDate =
                lastReviewDate && lastReviewDate > reviewDate
                    ? lastReviewDate
                    : reviewDate;
            firstApprovalDate =
                firstApprovalDate && firstApprovalDate < reviewDate
                    ? firstApprovalDate
                    : reviewDate;
    ...
    return {
        lastReviewDate,
        firstApprovalDate,
        reviewersWithStaleReviews: Array.from(
            staleReviewAuthorsByCommit.entries()
        ).map(([commit, info]) => ({
...
function getDangerLevel(pkgInfo: readonly PackageInfo[]): DangerLevel {
    ...
    if (pkgInfo.length === 0) {
        throw new Error(
            'Internal Error: not infrastructure but no packages touched too'
        );
    }
    const nonTestPackagesTouched = noNulls(
        pkgInfo.map(
            (p) => p.name === null || !p.files.some((f) => f.kind === 'test')
        )
    );
    if (nonTestPackagesTouched.length > 1) {
        return 'MultiplePackagesEdited';
    } else if (
        pkgInfo.some((p) => p.files.some((f) => f.kind === 'package-meta'))
    ) {
...
function getCIResult(
    headCommit: PR_repository_pullRequest_commits_nodes_commit
) {
    let ciUrl: string | undefined = undefined;
    let ciResult: CIResult = undefined!;

    const totalStatusChecks = headCommit.checkSuites?.nodes?.find((check) =>
...
async function getOwnersOfPackage(
    packageName: string,
    version: string,
    fetchFile: typeof defaultFetchFile
    ...
    try {
        ...
        if (e instanceof Error)
            return new Error(`error parsing owners: ${e.message}`);
    }
