...
import * as cachedQueries from '../utils/cacheQueries';
...
async function main() {
    ...
    while (
        !fs.existsSync(
            (dataPath = path.join(base, 'src', 'test', 'cachedQueries.json'))
        )
    ) {
    ...
    for (const query in cachedQueries) {
        data[query] = await cachedQueries[
            query as keyof typeof cachedQueries
        ]();
    }
