...
import { getProcessManyPRs } from '../graphql/queries/process-many-query';
...
async function main({ dateRange, dry }: ProcessManyOptions) {
    ...
    try {
        for (const pr of prs) {
            console.log(
                `Processing #${pr} (${prs.indexOf(pr) + 1} of ${prs.length})...`
            );
...
function reportResults(
    results: { pr: number; mutations: string[] }[],
    prCount: number,
    dry?: boolean
) {
    console.log(
        (dry ? 'Found' : 'Performed') +
            ` actions for ${results.length} of ${prCount} recently updated PRs.`
    );
    console.log('');
    for (const { pr, mutations } of results) {
        console.log(
            `=== https://github.com/DefinitelyTyped/DefinitelyTyped/pull/${pr} ===`
        );
...
if (!module.parent) {
    ...
    if (hoursAgoIndex < 0)
        throw new Error(
            `Must supply '--since-hours-ago' argument. 'npm run many -- --since-hours-ago 24`
        );
    const hoursAgo = +process.argv[hoursAgoIndex + 1];
    if (isNaN(hoursAgo))
        throw new Error(
            `Could not parse '${process.argv[hoursAgoIndex + 1]}' as a number.`
        );
