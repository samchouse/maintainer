...
export default async function main(
    ...
    let fixturePath = join('src', 'test', 'fixtures', directory);
    fixturePath = fixturePath.replace("'", '');
    const prNumber = parseInt(directory, 10);
    if (isNaN(prNumber))
        throw new Error(`Expected ${directory} to be parseable as a PR number`);
    ...
    if (overwriteInfo || !existsSync(jsonFixturePath)) {
        ...
        writeFileSync(
            jsonFixturePath,
            scrubDiagnosticDetails(JSON.stringify(response, null, '  '))
        );
    } else {
    ...
    const shouldOverwrite = (file: string) =>
        overwriteInfo || !existsSync(file);

    const derivedInfo = await deriveStateForPR(
        response,
        shouldOverwrite(filesJSONPath)
            ? initFetchFilesAndWriteToFile()
            : getFilesFromFile,
        shouldOverwrite(downloadsJSONPath)
            ? initGetDownloadsAndWriteToFile()
    ...
    writeFileSync(
        derivedFixturePath,
        scrubDiagnosticDetails(JSON.stringify(derivedInfo, null, '  '))
    );
    ...
    writeFileSync(
        resultFixturePath,
        scrubDiagnosticDetails(JSON.stringify(actions, null, '  '))
    );

    const mutationsFixturePath = join(fixturePath, 'mutations.json');
    const mutations = await executePrActions(
        actions,
        response.data,
        /*dry*/ true
    );
    writeFileSync(
        mutationsFixturePath,
        scrubDiagnosticDetails(
            JSON.stringify(
                mutations.map((m) => JSON.parse(m)),
                null,
                '  '
            )
        )
    ...
    async function getDownloadsAndWriteToFile(packageName: string) {
        downloadsFetched[packageName] = await getMonthlyDownloadCount(
            packageName
        );
        writeFileSync(
            downloadsJSONPath,
            JSON.stringify(downloadsFetched, null, '  ')
        );
    ...
    function getTimeFromFile() {
        return new Date(
            JSON.parse(readFileSync(derivedFixturePath, 'utf8')).now
        );
    }
}
